...
from services.deep_reasoner import DeepReasoner
...
class StateManager:
    ...
    def _configure_graph(self):
        ...
        router = Router(settings.google_gemini_client)
        researcher = Researcher(settings.google_gemini_client, tools)
        deep_reasoner = DeepReasoner(settings.open_api_client, settings.google_gemini_client)
        reflector = StructuredOutputReflector(settings.open_api_client, settings.google_gemini_client)
        ...
        # Add edges to end
        self.graph_builder.add_edge("researcher", "reflector")  # Research path needs structuring
        self.graph_builder.add_edge("reflector", END)
        self.graph_builder.add_edge("deep_reasoner", END)  # Reasoner output is already structured

        # Set entry point
        self.graph_builder.add_conditional_edges(START, router.route, {
