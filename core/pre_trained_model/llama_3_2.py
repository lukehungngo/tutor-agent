...
class Llama32Model:
    """Standalone implementation for Meta's Llama 3.2 model with robust JSON support."""
    ...
    def _setup_model(self):
        ...
        try:
            ...
            # # Handle special tokens for Llama
            if self.tokenizer.pad_token_id is None:
                # If pad_token is not set, use eos_token
                self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
                logger.info(
                    f"Setting pad_token_id to eos_token_id: {self.tokenizer.pad_token_id}"
                )
    ...
    def _format_prompt_for_json(self, prompt: str, schema: Dict) -> str:
        ...
        formatted_prompt = (
            f"{prompt}\n\n"
            f"RESPONSE FORMAT:\n{schema_str}\n\n"
            f"IMPORTANT: You must respond with a single valid JSON object. "
            f"Do not include any text before or after the JSON. "
            f"Do not use duplicate keys in the JSON."
        )
    ...
    def generate_text(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            if (
                ...
                and "<start_of_turn>model" in response
            ):
...
if __name__ == "__main__":
    ...
    try:
        ...
        text_response = llama.generate(prompt)
