  ...
  "question_potential": "Brief explanation of what kinds of questions could be generated from this chunk"
    ...
    def __init__(self, model_name: str = "gpt-4"):
        ...
        self.chunk_analysis_chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplate(
                input_variables=["chunk_content"], template=self.CHUNK_ANALYSIS_PROMPT
            ),
        )
    ...
    async def analyze_document(
        self,
        document_samples: List[Document],
        strategy: str = "holistic",
        user_highlights: Optional[Dict] = None,
    ) -> Dict:
        """
    ...
    async def _analyze_holistic(self, document_samples: List[Document]) -> Dict:
        ...
        samples_text = "\n\n---\n\n".join(
            [doc.page_content for doc in document_samples[:8]]
        )
        ...
        try:
            ...
            return {
                ...
                "document_sections": [
                    {"section": "Main Content", "query": "main content"}
                ],
            }
    ...
    async def _analyze_chunk_based(self, document_chunks: List[Document]) -> Dict:
        ...
        for chunk in chunks_to_analyze:
            try:
                response = await self.chunk_analysis_chain.ainvoke(
        ...
        for analysis in chunk_analyses:
            ...
            if topic in topics_map:
                # Merge key concepts
                topics_map[topic]["key_concepts"].extend(
                ...
                if importance_values.get(new_importance, 0) > importance_values.get(
        ...
        for topic in topics_map:
            topics_map[topic]["key_concepts"] = list(
                set(topics_map[topic]["key_concepts"])
            )
    ...
    async def _analyze_user_assisted(
        self, document_samples: List[Document], user_highlights: Dict
    ) -> Dict:
        """
        ...
        for section in highlighted_sections:
            document_sections.append(
                {
                    "section": section.get("text", "")[:50]
                    + "...",  # Use beginning of text as section name
    ...
    def get_document_samples(
        self, vector_store, max_samples: int = 8
    ) -> List[Document]:
        """Get representative samples from different parts of the document."""
        # Beginning samples
        beginning_docs = vector_store.similarity_search(
            "introduction beginning overview", k=2
        )
